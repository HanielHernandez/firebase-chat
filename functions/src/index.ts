...
export const getUserNyPhoneNumber = async (phoneNumber: string): Promise<any> => {
    const response = await admin.firestore().collection('users').where('phoneNumber', '==', phoneNumber).limit(1).get()
...
exports.addConversation = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (!recipient) {
            ...
            const writeResult = await admin.firestore().collection('conversations').add(conv)
    ...
    .onCreate(async (snap, context): Promise<boolean> => {
        ...
        try {
            ...
            if (exitsConv.docs.length > 0) {
                console.log('Ya existe una conversación con recipient id ' + newConv.senderId)
                return false
            } else {
                ...
                console.log('Se creo un conversación de recipient con id ' + recipientConv.id)
            }
        } catch (e) {
            console.error(e)
        }
...
exports.onUserUpdate = functions.firestore.document('users/{userId}').onUpdate(async (change, context) => {
    ...
    console.log('updating usersUpdatefunction')
    const newProfileImageURL = change.after.data().profileImageUrl
    const conversationsRef = admin.firestore().collection('conversations')
    const userConversations = await conversationsRef.where('recipient.id', '==', userId).get()
    ...
    userConversations.forEach((doc) => {
        const conversationRef = conversationsRef.doc(doc.id)
        batch.update(conversationRef, {
            conversationImageUrl: newProfileImageURL,
            'recipient.profileImageUrl': newProfileImageURL
        })
    })
    ...
    // get all converstions with recipient.id set as user
})
